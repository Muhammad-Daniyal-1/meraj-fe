...
import { AgentFormData, AgentSchema } from "@/lib/schema";
import { useUpdateAgentMutation, useGetAgentQuery } from "@/lib/api/agentApi";
import { toast } from "react-toastify";
import { useRouter } from "next/navigation";

export default function EditAgentForm({ id }: { id: string }) {
  ...
  const { data, isLoading: isGetAgentLoading } = useGetAgentQuery(id);
  ...
  const onSubmit = async (formData: AgentFormData) => {
    try {
      await updateAgent({ ...formData, _id: id }).unwrap();
      toast.success("Agent updated successfully!");
      router.push("/dashboard/agents");
      reset();
    } catch (err: any) {
      console.log("Error updating agent:", err);
      toast.error(err?.data?.message || "Failed to update agent.");
    }
  };
